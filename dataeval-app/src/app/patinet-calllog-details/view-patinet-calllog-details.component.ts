...
@Component({
  selector: 'app-view-patinet-calllog-details',
  templateUrl: './view-patinet-calllog-details.component.html',
  styleUrls: ['./view-patinet-calllog-details.component.css']
})
export class ViewPatinetCalllogDetailsComponent implements AfterViewInit, OnInit {
  ...
  constructor(private service: PatientCallLogService,private userService: UserService, private roleService: RoleService,
    ...
    private commonService: CommonService, private router: Router, private authenticationService: AuthenticationService, private route: ActivatedRoute) { 
  ...
  ngOnInit() {
    ...
    this.createForm = new FormGroup({
      ...
      discusstionDate:new FormControl() 
    }); 
        ...
        this.service.getByPatient(this.id) 
          .subscribe((response:PatientCallLog) => { 
            ...
            this.createForm.patchValue({
              ...
              staffFName:   (response.careTeamMemberFirstName),
              staffLName:  (response.careTeamMemberLastName),
              ...
              discusstionDate:new Date(response.creationDate).toISOString().slice(0,16),
              nextAptDate: new Date(response.nextMonthAppointmentDate).toISOString().slice(0,16)
            });
